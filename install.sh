...
__     __ ___  _   _     _     _   _   ___   ____  _____ __     __ _   _ 
\ \   / /|_ _|| \ | |   / \   | | | | / _ \ / ___||_   _|\ \   / /| \ | |
 \ \ / /  | | |  \| |  / _ \  | |_| || | | |\___ \  | |   \ \ / / |  \| |
  \ V /   | | | |\  | / ___ \ |  _  || |_| | ___) | | | _  \ V /  | |\  |
...
function get_hdsentinel()
{
	...
	local -r URL="https://files.vinahost.vn/VinaHost_Files/hdsentinel"
...
{
	local -r HOME_FOLDER="/vinahost"
	local -r URL="https://raw.githubusercontent.com/tamdm/bash/master/collect.sh"
	if [[ ! -f "${HOME_FOLDER}/collect.sh" ]]
	then
		$( curl -m 5 -L ${URL} --output ${HOME_FOLDER}/collect.sh && chmod +x ${HOME_FOLDER}/collect.sh ) || $(wget --no-check-certificate -O ${HOME_FOLDER}/collect.sh ${URL} && chmod +x ${HOME_FOLDER}/collect.sh)
	else 
		printf "File collect.sh is exist. Not to download new file.\n"
	fi
}
