...
{	
	...
	case $opt in 
		Opt_1)
			echo -e "Set crontab run every 12 hours: 0 */12 * * * /bin/bash /vinahost/send_alert.sh"
			read -p "Press Y/y to set cronjob every 12 hours - Press N/n to cancel:" -n 1 -r
			echo
			[[ "${REPLY}" =~ ^[Yy]$ ]] && $(( crontab -l ; echo "0 */12 * * * /bin/bash /vinahost/send_alert.sh") | awk '!x[$0]++'| crontab - )
			;;
		Opt_2)
